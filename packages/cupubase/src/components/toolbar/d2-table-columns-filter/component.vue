<template>
  <el-drawer :title="_text.title" :visible.sync="active" size="300px" class="d2p-table-columns-filter" append-to-body>
    <div class="d2p-drawer-wrapper">
      ...
      <el-card shadow="never">
        <div class="component--list">
          <div key="__first__" class="component--list-item" flex="main:justify cross:center">
            <span :span="12">
              <el-checkbox v-model="checkAll" :indeterminate="isIndeterminate" @change="onCheckAllChange"> {{ showLength }} / {{ options.length }} </el-checkbox>
            </span>
            <span class="title">{{ _text.fixed }} / {{ _text.order }}</span>
          </div>
          <draggable v-model="currentValue" ghost-class="ghost" handle=".handle">
            <transition-group>
              <div v-for="(option, index) of currentValue" :key="option.key" class="component--list-item" flex="main:justify cross:center">
                <el-checkbox v-model="currentValue[index].show" flex-box="1">
                  {{ option.title || option.key || _text.unnamed }}
                </el-checkbox>
                <d2-table-columns-fixed-controller v-model="currentValue[index].fixed" flex-box="0" class="d2-mr-10" @change="value => fixedChange(index, value)" />
      ...
      <el-row class="drawer-footer" :gutter="10">
        <el-col :span="12">
          <d2-button size="default" icon="el-icon-refresh" :label="_text.reset" block @click="reset" />
        </el-col>
        <el-col :span="12">
          <d2-button size="default" type="primary" icon="el-icon-check" :label="_text.confirm" block @click="submit()" />
        </el-col>
      </el-row>
    </div>
  </el-drawer>
...
export default {
  ...
  computed: {
    ...
    _text() {
      const def = {
        title: this.$t('baseCommon.component.columnsSetBtn'),
        fixed: this.$t('baseCommon.component.fixed'),
        order: this.$t('baseCommon.component.order'),
        reset: this.$t('baseCommon.component.reset'),
        confirm: this.$t('baseCommon.component.confirm'),
        unnamed: this.$t('baseCommon.component.unnamed')
      }
  ...
  methods: {
    ...
    fixedChange(index, value) {
      if (value) {
        this.currentValue[index].show = true
      }
      if (value === "left") {
