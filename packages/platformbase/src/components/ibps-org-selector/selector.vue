<template>
  <div>
    ...
    <!-- 选择器 -->
    <ibps-org-selector-dialog :visible="selectorVisible" :value="selectorValue" :levelFilter="levelFilter" :defaultExpandAll="defaultExpandAll" :multiple="multiple" @close="visible => (selectorVisible = visible)" @action-event="handleSelectorActionEvent" />
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    items() {
      ...
      if (this.multiple) {
        if (this.selectorValue.constructor === 'String') {
          this.selectorValue.split(',')
        }
  ...
  methods: {
     ...
     */
    getDataInfo(id) {
      remoteRequest('org' + this.valueKey, id, () => {
    ...
    getRemoteByIdFunc(id) {
      return new Promise((resolve, reject) => {
        if (this.valueKey === 'id') {
          ...
          window.apiList['org/entity'].getByAlias({
            alias: id
          })
            .then(response => {
              resolve(response)
