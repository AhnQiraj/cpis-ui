<template>
  <div class="query-builder form-inline" style="padding: 0 10px">
    <setting-builder-group :index="0" :rules.sync="rules" :filters="mergedFilters" :max-depth="maxDepth" :depth="depth" :labels="labels" :conditions="mergedConditions" :children-key="childrenKey" :condition-key="conditionKey" :sort="sort" type="setting-builder-group" @errors="handleErrors" @child-deletion-requested="remove" />
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    if (typeof this.value !== 'undefined') {
      this.rules = Object.assign(this.rules, this.setRules(this.value))
    }
  },
  beforeDestroy() {
    this.rules = null
  ...
  methods: {
    setRules(val) {
      ...
      if (this.$utils.isEmpty(val.rules)) {
        return val
      }
      this.setRuleId(val)
      return val
    ...
    setRuleId(_val) {
      for (let i = 0; i < _val.rules.length; i++) {
        if (this.$utils.isEmpty(_val.rules[i].rule_id)) {
          _val.rules[i].rule_id = new Ids([32, 36, 1]).next()
        }
        if (this.$utils.isArray(_val.rules[i].rules) && _val.rules[i].rules.length > 0) {
          this.setRuleId(_val.rules[i])
        }
      }
    },
    handleMergedFilters(filters) {
      ...
      filters.forEach(filter => {
        ...
        if (!filter.input) {
          filter.input = QueryBuilder.TYPES[filter.type] === 'number' ? 'number' : QueryBuilder.TYPES[filter.type] === 'datetime' ? 'date' : 'text'
          if (filter.input === 'date') {
            filter.input = this.setFilterType(filter.input, filter.format)
          }
    ...
    // 细分日期的type
    setFilterType(type, format) {
      if (this.$utils.isEmpty(format)) {
        return type
      }
      return this.$utils.getTypeByFormat(format)
    },
    /**
     * 构建条件类型
     */
