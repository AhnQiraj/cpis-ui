    ...
    <div class="rules-group-header">
      ...
      <div class="btn-group pull-right group-actions">
        <el-button-group>
          ...
          <el-button size="mini" type="warning" icon="el-icon-plus" @click="addScript">{{ labels.addScript }}</el-button>
    ...
    <div class="rules-group-body">
      <div class="rules-list">
        <template v-for="(child, i) in rulesData[childrenKey]">
          <component
            :is="child && child.condition?'setting-builder-group':'setting-builder-rule'"
            ...
            :sort="sort"
            :script-type="rulesData[childrenKey][i].ruleType === '2'?scriptType:''"
            :script-type-title="rulesData[childrenKey][i].ruleType === '2'?scriptTypeTitle:''"
...
import ScriptDialog from '@platform/business/platform/bpmn/setting/bpmn-setting/components/user-setting/user-dialog'
import Ids from 'ids'
export default {
  ...
  methods: {
    ...
    setDefault() {
      this.defaultRule = JSON.parse(JSON.stringify({
        ...
        type: this.filters[0].type || 'varchar',
        input: this.filters[0].input,
        operator: this.filters[0].operators[0],
        source: this.filters[0].sources[0],
        value: null,
        ruleType: '1',
        key: this.filters[0].key,
        name: this.filters[0].name,
        tableName: this.filters[0].tableName,
        attrType: this.filters[0].attrType
      }))
    ...
    handleData(data) {
      ...
      const _scriptRule = {
        conDesc: _data.description,
        ruleType: '2',
        script: _data.script,
        rule_id: new Ids([32, 36, 1]).next()
      }
      this.rulesData[this.childrenKey].push(_scriptRule)
    },
    // 添加组
    addGroup() {
