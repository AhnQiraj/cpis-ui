...
import fecha from '@platform/utils/fecha'
import FormulaUtil from './formula'
import FormFieldUtil from './formFieldUtil'
import AddressUtls from '@platform/utils/address'
import { dataFormatMap } from './fieldOptions'
import FormOptions from './formOptions'
import { validateRequired, validateLengthRange, validateOptions, validateInteger, validateDecimal, validateNumberRange } from '@platform/utils/validate'
...
const FormUtil = {
  ...
  async runCalDynamic(self, responseDynamics, field, row) {
    if (Utils.isEmpty(responseDynamics)) {
      return self.models
    }
      ...
      remoteRequest(
        ...
        () => {
          return new Promise((rev, rej) => {
            window.apiList['form/common']
              .getScriptValue(params)
              .then(response => {
                rev(response)
              })
              ['catch'](error => {
  ...
  runCalFormula(self, responseFormulas, curCode, row) {
    if (Utils.isEmpty(responseFormulas)) {
      return self.models
    }
      ...
      if (/^\$(_widget_)/.test(n)) {
        ...
        if (Utils.isEmpty(h)) {
          ...
          if (mainCode === calCode) {
            ...
            // 其他子表
            if (isSameTable && Utils.isNotEmpty(row)) {
              // 相同的子表
  ...
  runLinkage: function (self, responseLinkages, field, row) {
    ...
    for (let i = 0; i < responseLinkages.length; i++) {
      ...
      // 加载数据
      window.apiList['data/dataTemplate']
        .queryLinkageData(ActionUtils.formatParams(formParams))
        .then(response => {
  ...
  getCurrentDataValue({ selectorType, bindId, fieldName, storeType }) {
    const userInfo = Vue.prototype.store.getters.userInfo
    // 缓存用户
  ...
  async getFieldDefaultValue(field, params = {}) {
    ...
    if (defaultValueType === 'fixed') {
      if (fieldType === 'number' || fieldType === 'inputNumber' || fieldType === 'slider' || fieldType === 'rate') {
        // 单选，复选，下拉
      ...
      } else if (fieldType === 'currentUser' || fieldType === 'currentOrg') {
        // 当前用户、组织
      ...
      if (fieldType === 'selector' || fieldType === 'currentUser' || fieldType === 'currentOrg') {
        // 当前用户、组织
        result = this.getCurrentDataValue({
      ...
      // =================  当前时间   =================
    } else if (defaultValueType === 'today') {
      // 当前时间
      result = this.getFieldDatefmtValue(fieldOptions)
    }
    return result
  ...
  hiddenApprovalOpinionRigths(field, params) {
    ...
    if (Utils.isNotEmpty(nodeId)) {
      ...
      if (Utils.isEmpty(formOpinionSetting)) {
        return false
      }
      const fieldName = field.name
  ...
  initFormOpinionData: function (attrs) {
    ...
    for (let i = 0; i < opinionList.length; i++) {
      ...
      if (Utils.isEmpty(opinion.completeTime)) {
        continue
      }
      formOpinionList.push(opinion)
    }
    if (Utils.isEmpty(formOpinionConfig)) {
      // 没有绑定节点的
      ...
      for (const k in formOpinionConfig) {
        ...
        if (Utils.isEmpty(nodes) || nodes.includes('')) {
          // 全局节点
  ...
  buildFormRules(field, required, models) {
    ...
    //  最大、最小字符串长度验证
    if ((fieldOptions.is_min_length && Utils.isNotEmpty(fieldOptions.min_length)) || (fieldOptions.is_max_length && Utils.isNotEmpty(fieldOptions.max_length))) {
      rules.push({
    ...
    //  最大、最小值验证[数字]
    if ((fieldOptions.is_min && Utils.isNotEmpty(fieldOptions.min)) || (fieldOptions.is_max && Utils.isNotEmpty(fieldOptions.max))) {
      rules.push({
    ...
    if (Utils.isNotEmpty(fieldOptions.start_date_type) || Utils.isNotEmpty(fieldOptions.end_date_type)) {
      rules.push({
    ...
    //  最多、最少选项验证
    if ((fieldOptions.is_min_mum && Utils.isNotEmpty(fieldOptions.min_mum)) || (fieldOptions.is_max_mum && Utils.isNotEmpty(fieldOptions.max_mum))) {
      rules.push({
  ...
  getListLinkDynamicParams(fieldOptions, data = {}, formFieldMap = {}) {
    ...
    for (let i = 0; i < conditions.length; i++) {
      ...
      if (Utils.isNotEmpty(changeValueName) && condition.mode === 'bind') {
        // 绑定表单
  ...
  setCascadeData(data, cascade, refs) {
    if (cascade.fieldType === 'onlineForm' || cascade.fieldType === 'detailForm' || cascade.fieldType === 'component') {
      // 处理表单类型
      const formField = refs[0].$refs.formField.$children
      if (Utils.isNotEmpty(formField)) {
    ...
    } else if (cascade.fieldType === 'dataTemplate') {
      // 数据模版
