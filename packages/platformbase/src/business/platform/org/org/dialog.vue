<template>
  <ibps-selector-dialog :value="selectedValue" :visible="selectorVisible" :label-key="labelKey" width="60%" v-bind="$props" @input="value => (selectedValue = value)" @remove-select="removeSelect" @close="handleClose" @action-event="handleActionEvent">
    <template slot-scope="scope">
      <ibps-selector-panel ref="panel" v-bind="scope" :party-type-id="partyTypeId" :current-org-id="currentOrgId" :script="script" :form-data="formData" :store="store" :pk-key="store === 'bind' ? 'name' : 'id'" :is-use-scope="isUseScope" @selected="value => (selectedValue = value)" />
    </template>
