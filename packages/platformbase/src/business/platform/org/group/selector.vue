<template>
  <div>
    ...
    <!-- 选择器 -->
    <ibps-group-selector-dialog :visible="selectorVisible" :value="selectorValue" :multiple="multiple" @close="visible => (selectorVisible = visible)" @action-event="handleSelectorActionEvent" />
  </div>
</template>
<script>
import { get, getByKey } from '@platform/api/platform/org/group'
...
export default {
  ...
  methods: {
     ...
     */
    getDataInfo(id) {
      remoteRequest('group' + this.valueKey, id, () => {
      ...
      return new Promise((resolve, reject) => {
        if (this.valueKey === 'id') {
          get({
            ...
            .catch(() => {})
        } else if (this.valueKey === 'groupAlias') {
          getByKey({
            groupKey: id
          })
            .then(response => {
