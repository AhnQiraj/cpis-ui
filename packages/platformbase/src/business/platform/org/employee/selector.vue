<template>
  <div>
    ...
    <!-- 选择器 -->
    <ibps-employee-selector-dialog :visible="selectorVisible" :value="selectorValue" :multiple="multiple" :class="className" :party-type-id="partyTypeId" :current-org-id="currentOrgId" @close="visible => (selectorVisible = visible)" @action-event="handleSelectorActionEvent" />
  </div>
</template>
<script>
import { get } from '@platform/api/platform/org/employee'
import { getByAccount } from '@platform/api/platform/org/user'
