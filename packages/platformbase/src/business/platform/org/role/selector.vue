<template>
  <div>
    ...
    <!-- 选择器 -->
    <ibps-role-selector-dialog :visible="selectorVisible" :value="selectorValue" :multiple="multiple" @close="visible => (selectorVisible = visible)" @action-event="handleSelectorActionEvent" />
  </div>
</template>
<script>
import { get, getByAlias } from '@platform/api/platform/org/role'
...
export default {
  ...
  methods: {
     ...
     */
    getDataInfo(id) {
      remoteRequest('role' + this.valueKey, id, () => {
