<template>
  <ibps-selector-dialog :value="selectedValue" :visible="selectorVisible" :label-key="labelKey" v-bind="$props" @input="value => (selectedValue = value)" @remove-select="setSelectRow" @close="handleClose" @action-event="handleActionEvent">
    <template slot-scope="scope">
      <ibps-selector-panel ref="panel" v-bind="scope" :is-auth="isAuth" :org-id="orgId" :party-type-id="partyTypeId" :current-org-id="currentOrgId" :pk-key="store === 'bind' ? 'name' : 'id'" :script="script" :form-data="formData" :is-use-scope="isUseScope" @selected="value => (selectedValue = value)" />
    </template>
