        ...
        <div>
          <el-form label-position="top" @submit.native.prevent>
            <el-form-item class="form-script-head-title">
              <label slot="label">脚本
                &nbsp;&nbsp;
                <el-button type="text" @click="handleDefaultScript">重置默认脚本</el-button>
              </label>
            ...
            <div class="form-script-foot">
              <ul>
                <li>请从左侧面板选择的字段名</li>
                <li>默认脚本<span class="red">function(data){}</span>的参数<span class="red">data</span>是分组的数据，格式为对象数组。</li>
                <li>请在默认脚本<span class="red">function(data){}</span>中编写内容，并返回结果</li>
...
export default {
  ...
  data() {
    ...
    return {
      dialogVisible: false,
      formScript: '',
      defaultValue: 'function(data){ // data是列表分组后的当前组的数据,格式为数组\n    \n    return data\n}',
      cmOption: {
        tabSize: 4,
        lineNumbers: true,
        line: true,
  ...
  methods: {
    getFormData() {
      this.$nextTick(() => {
        this.formScript = this.data || this.defaultValue
      })
    },
    getEditor() {
